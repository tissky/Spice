...
class StubMemoryProvider(MemoryProvider):
    def __init__(self) -> None:
        self.records_by_namespace = {
            ...
            "general.evolution": [{"id": "memory.evolution.1", "summary": "User asked why not B"}],
            "general.delegated_perception": [
                {"id": "memory.delegated.1", "summary": "Hermes reported external findings"}
            ],
...
class MemoryContextSchemaTests(unittest.TestCase):
    ...
    def test_simulation_context_has_general_runtime_fields(self) -> None:
        context = SimulationContext.create(
            ...
            executor_capabilities={
                "executor_id": "codex",
                "source": "static_baseline",
                "capability_ids": ["workspace_write"],
            },
        ...
        self.assertIn("workspace_write", context.executor_capabilities["capability_ids"])
    ...
    def test_reflection_context_has_general_runtime_fields_with_empty_defaults(self) -> None:
        ...
        self.assertEqual(context.executor_capabilities, {})
...
class DeterministicGeneralContextCompilerTests(unittest.TestCase):
    ...
    def test_compile_general_decision_context_carries_workspace_perception_context(self) -> None:
        provider = StubMemoryProvider()
        compiler = DeterministicContextCompiler(memory_provider=provider)
        workspace_artifact = build_workspace_perception_artifact(
            workspace_root="/repo",
            trigger="new_decision",
            queries=[{"query": "current state-as-context implementation"}],
            files_read=[{"path": "spice/memory/context.py", "chars_read": 900}],
            files_skipped=[{"path": ".git/config", "reason": "deny_dir"}],
            facts=[
                {
                    "text": "DecisionContext already has a workspace_context field.",
                    "source_path": "spice/memory/context.py",
                }
            ],
            summary="Decision context has a workspace_context slot.",
        )

        context = compiler.compile_general_decision_context(
            _world_state(),
            _general_state(),
            current_intent="Look at the repo before deciding.",
            active_decision_frame=_active_frame(),
            session=_session(),
            config=_config(),
            workspace_context=workspace_context_from_perception(workspace_artifact),
        )

        self.assertEqual(context.workspace_context["memory_provider"], "file")
        self.assertEqual(context.workspace_context["source"], "workspace_perception")
        self.assertEqual(context.workspace_context["perception_id"], workspace_artifact.perception_id)
        self.assertEqual(context.workspace_context["files_read"][0]["path"], "spice/memory/context.py")
        self.assertEqual(context.workspace_context["files_skipped"][0]["reason"], "deny_dir")
        self.assertIn("workspace_context", context.workspace_context["facts"][0]["text"])
        self.assertTrue(context.evidence_context["workspace"]["present"])
        self.assertEqual(
            context.evidence_context["workspace"]["perception_id"],
            workspace_artifact.perception_id,
        )
        self.assertEqual(context.evidence_context["workspace"]["source_count"], 1)
        self.assertFalse(context.evidence_context["url"]["present"])
    ...
    def test_compile_general_simulation_context_serializes_candidates(self) -> None:
        ...
        context = compiler.compile_general_simulation_context(
            ...
            workspace_context={
                ...
                "perception_id": "workspace.simulation",
                "summary": "Simulation should see compact repo facts.",
            },
        )
        ...
        self.assertEqual(context.workspace_context["perception_id"], "workspace.simulation")
        self.assertEqual(context.evidence_context["workspace"]["perception_id"], "workspace.simulation")
        self.assertEqual(context.executor_affordance["permission"], "workspace_write")
        self.assertEqual(context.executor_capabilities["executor_id"], "codex")
        self.assertIn("test_run", context.executor_capabilities["capability_ids"])
        self.assertEqual(
            [record["id"] for record in context.retrieved_memory],
            [
                "memory.simulation.1",
    ...
    def test_compile_general_reflection_context_carries_execution_artifacts(self) -> None:
        ...
        context = compiler.compile_general_reflection_context(
            ...
            workspace_context={
                "source": "workspace_perception",
                "perception_id": "workspace.reflection",
                "summary": "Reflection should carry the repo facts used by the handoff.",
            },
        )
        ...
        self.assertEqual(context.workspace_context["perception_id"], "workspace.reflection")
        self.assertEqual(context.evidence_context["workspace"]["perception_id"], "workspace.reflection")
        self.assertEqual(context.executor_capabilities["executor_id"], "codex")
...
def _config() -> dict:
    return {
        ...
        "executor_capabilities": {
            ...
            "summary": "Good for repo-local coding work.",
