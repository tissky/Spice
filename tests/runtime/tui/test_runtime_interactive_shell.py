...
class RuntimeInteractiveShellTests(unittest.TestCase):
    ...
    def test_shell_deterministic_detail_commands_use_latest_run(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            setup_workspace(project_root=tmp_dir)
            output = io.StringIO()
            ...
            run_interactive_shell(
                project_root=tmp_dir,
                input_stream=io.StringIO(
                    "Review this repo and pick the safest next action.\n"
                    "/card\n"
                    "/why\n"
                    "/sim\n"
                    "/details\n"
            ...
            text = output.getvalue()
            self.assertIn("DECISION COMPARISON", text)
            self.assertIn("WHY THIS DECISION", text)
            self.assertIn("SIMULATION SUMMARY", text)
            self.assertIn("ACTIVE DECISION FRAME", text)
            self.assertIn('"path_type": "manual_intent_run_once"', text)
