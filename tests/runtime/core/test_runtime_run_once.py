...
def _configure_codex_executor_for_tests(
    project_root: str,
    *,
    extra_capabilities: list[str] | None = None,
) -> None:
    update_workspace_config(project_root, "executor", "codex")
    update_workspace_config(project_root, "executor_command", sys.executable)
    update_workspace_config(project_root, "executor_permission_mode", "workspace_write")
    if extra_capabilities:
        ...
        payload["executor_capabilities"] = {
            "schema_version": "spice.executor_capability_snapshot.v1",
            "executor_id": "codex",
            "provider": "codex",
            "status": "available",
            ...
            "summary": "Test executor capability snapshot.",
            "limitations": [],
        }
...
class RuntimeRunOnceTests(unittest.TestCase):
    def test_run_once_persists_state_run_and_decision_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            turn_payload = store.load_conversation_turn(result.artifact["conversation_turn_id"])
            ...
            self.assertIn("conversation_turn", run_payload["store_paths"])
            self.assertTrue((Path(tmp_dir) / run_payload["store_paths"]["conversation_turn"]).exists())
            self.assertTrue(result.artifact["conversation_turn_id"].startswith("turn."))
            self.assertTrue(result.artifact["response_id"].startswith("response."))
            self.assertEqual(run_payload["conversation_route"], "new_decision")
            self.assertEqual(turn_payload["turn_id"], result.artifact["conversation_turn_id"])
            self.assertEqual(turn_payload["response_id"], result.artifact["response_id"])
            self.assertEqual(turn_payload["route"], "new_decision")
            self.assertEqual(turn_payload["session_id"], "session.default")
            self.assertEqual(turn_payload["user_input"], "Review this repo and suggest the safest next action")
            self.assertEqual(turn_payload["source_run_id"], result.artifact["run_id"])
            self.assertEqual(turn_payload["source_decision_id"], result.artifact["decision_id"])
            self.assertEqual(turn_payload["source_candidate_id"], result.artifact["selected_candidate_id"])
            self.assertIn("run", turn_payload["artifact_refs"])
            self.assertIn("decision", turn_payload["artifact_refs"])
            ...
            self.assertIn(
                ...
                run_payload["session"]["conversation_turn_ids"],
            )
            ...
            self.assertEqual(memory["evidence_context"]["confidence"], "none")
            self.assertEqual(
            ...
            self.assertEqual(len(evolution_records), 1)
            evolution = evolution_records[0]
            self.assertEqual(evolution["route"], "new_decision")
            self.assertEqual(evolution["user_input"], "Review this repo and suggest the safest next action")
            self.assertEqual(evolution["decision_id"], result.artifact["decision_id"])
            self.assertEqual(evolution["candidate_id"], result.artifact["selected_candidate_id"])
            self.assertEqual(evolution["turn_id"], result.artifact["conversation_turn_id"])
            self.assertTrue(evolution["response_summary"])
            self.assertEqual(evolution["evidence_context"]["confidence"], "none")
            self.assertEqual(result.artifact["evolution_memory_writeback"]["status"], "written")
            self.assertEqual(
                result.artifact["evolution_memory_writeback"]["namespace"],
                "general.evolution",
            )
    ...
    def test_run_once_artifact_is_readable_and_does_not_execute(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(
                artifact["compiled_context"]["decision_context"]["executor_capabilities"]["executor_id"],
                "dry_run",
            )
            self.assertEqual(
                artifact["compiled_context"]["simulation_context"]["executor_capabilities"]["source"],
                "static_baseline",
            )
            self.assertTrue(
    ...
    def test_run_once_passes_compiled_context_to_llm_generation_and_simulation(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            def expand_side_effect(**kwargs):
                ...
                self.assertEqual(decision_context.executor_capabilities["executor_id"], "dry_run")
            ...
            def simulate_side_effect(**kwargs):
                ...
                self.assertEqual(simulation_context.executor_capabilities["source"], "static_baseline")
    ...
    def test_run_once_no_persist_skips_memory_writeback(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(result.artifact["evolution_memory_writeback"]["status"], "skipped")
            self.assertEqual(result.artifact["evolution_memory_writeback"]["reason"], "persist=false")
            ...
            self.assertEqual(evolution_records, [])
    ...
    def test_run_once_act_mode_selects_handoff_candidate_and_creates_approval(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            setup_workspace(project_root=tmp_dir)
            _configure_codex_executor_for_tests(tmp_dir)
            ...
            self.assertEqual(selected_affordance["executor"]["executor_id"], "codex")
    ...
    def test_run_once_records_runtime_required_capability_on_execution_candidates(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            setup_workspace(project_root=tmp_dir)

            result = run_once(
                "Fix the failing test by changing the repo files.",
                project_root=tmp_dir,
                now=NOW,
                run_intent_mode="act",
                full_loop_preview=False,
            ...
            execute_candidate = next(
                ...
                if candidate["action_type"] == "intent.execute"
            )
            self.assertEqual(execute_candidate["required_capability"], "code_edit")
            self.assertEqual(
                execute_candidate["metadata"]["required_capability_inference"]["source"],
                "runtime_inference",
            )
            compare_candidate = next(
                candidate
                for candidate in result.artifact["compare_payload"]["candidate_decisions"]
                if candidate["candidate_id"] == execute_candidate["candidate_id"]
            )
            self.assertEqual(compare_candidate["required_capability"], "code_edit")
            frame_candidate = next(
                candidate
                for candidate in result.artifact["active_decision_frame"]["candidates"]
                if candidate["candidate_id"] == execute_candidate["candidate_id"]
            )
            self.assertEqual(frame_candidate["required_capability"], "code_edit")

    def test_aha_demo_intent_generates_multi_candidate_approval_and_handoff(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            setup_workspace(project_root=tmp_dir)
            _configure_codex_executor_for_tests(tmp_dir, extra_capabilities=["github_work"])
