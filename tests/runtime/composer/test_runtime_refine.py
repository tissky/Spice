...
class RuntimeRefineTests(unittest.TestCase):
    def test_refine_adds_manual_candidate_and_saves_updated_decision(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            refine_evolution = next(
                record
                for record in evolution_records
                if record["run_id"] == result.artifact["run_id"]
            )
            self.assertEqual(refine_evolution["route"], "follow_up")
            self.assertEqual(refine_evolution["follow_up_type"], "refine")
            self.assertEqual(refine_evolution["decision_id"], result.artifact["decision_id"])
            self.assertEqual(refine_evolution["candidate_id"], result.artifact["selected_candidate_id"])
            self.assertEqual(refine_evolution["user_input"], "Consider rollback first, then fix forward.")
