...
class LLMProposalNormalizerTests(unittest.TestCase):
    ...
    def test_read_only_perception_mode_blocks_execution_requested_proposal(self) -> None:
        candidate = normalize_decision_proposal(
            {
                "title": "Inspect current repo implementation",
                "recommendation": "Read the repo and summarize implementation evidence.",
                "why_now": ["The user asked for code-backed analysis."],
                "expected_result": "A source-backed architecture recommendation.",
                "downside": "It consumes read-only perception budget.",
                "success_signal": "The answer cites files and implementation facts.",
                "risk_level": "medium",
                "execution_requested": True,
                "handoff_task": "Read the repo and decide what to build next.",
            },
            index=0,
            decision_mode="read_only_perception",
        ...
        self.assertEqual(candidate.side_effect_class, "read_only")
        self.assertEqual(candidate.execution_boundary.mode, "none")
        self.assertEqual(candidate.execution_boundary.protocol, "")
        ...
        self.assertTrue(
            candidate.metadata["execution_intent"]["metadata"][
                "read_only_intent_boundary_applied"
            ]
        )
