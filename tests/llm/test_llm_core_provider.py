...
class LLMCoreProviderTests(unittest.TestCase):
    ...
    def test_llm_client_stream_falls_back_to_generate(self) -> None:
        provider = DeterministicLLMProvider(
            responses={LLMTaskHook.ASSIST_DRAFT: '{"ok": true}'}
        )
        registry = ProviderRegistry.empty().register(provider)
        ...
        self.assertEqual(chunks[0].text, '{"ok": true}')
        self.assertEqual(chunks[0].finish_reason, "stop")
        self.assertEqual(chunks[0].raw_event["stream_fallback"], "generate")
    ...
    def test_openrouter_provider_streams_chat_completion_chunks(self) -> None:
        provider = OpenRouterLLMProvider()
        captured: dict[str, object] = {}
    ...
    def test_openai_provider_streams_chat_completion_chunks(self) -> None:
        provider = OpenAILLMProvider()
        captured: dict[str, object] = {}
    ...
    def test_anthropic_provider_stream_falls_back_to_generate(self) -> None:
        provider = AnthropicLLMProvider()
        ...
        with patch.dict(
            ...
            chunks = list(
                provider.stream(
                    LLMRequest(task_hook=LLMTaskHook.RESPONSE_COMPOSE, input_text="prompt"),
                    LLMModelConfig(
                        provider_id="anthropic",
                        model_id="claude-3-5-sonnet-latest",
                    ),
        ...
        self.assertEqual(len(chunks), 1)
        self.assertEqual(chunks[0].text, "fallback text")
        self.assertEqual(chunks[0].finish_reason, "end_turn")
        self.assertEqual(chunks[0].raw_event["stream_fallback"], "generate")
    ...
    def test_deepseek_provider_streams_chat_completion_chunks(self) -> None:
        provider = DeepSeekLLMProvider()
        captured: dict[str, object] = {}
                ...
                [
                    {
                        ...
                        "model": "deepseek-chat",
                        "choices": [{"delta": {"content": "ds"}, "finish_reason": "stop"}],
                    },
                ]
            )
        ...
        self.assertEqual(payload["model"], "deepseek-chat")
        self.assertEqual(payload["max_tokens"], 128)
        self.assertEqual([chunk.text for chunk in chunks], ["ds"])
        self.assertEqual(chunks[0].finish_reason, "stop")
    ...
    def test_mimo_provider_streams_chat_completion_chunks(self) -> None:
        provider = MiMoLLMProvider()
        captured: dict[str, object] = {}
                ...
                [
                    {
                        ...
                        "model": "mimo-v2.5-pro",
                        "choices": [{"delta": {"content": "mimo"}, "finish_reason": "stop"}],
                    },
                ]
            )
        ...
        self.assertEqual([chunk.text for chunk in chunks], ["mimo"])
        self.assertEqual(chunks[0].finish_reason, "stop")
