...
class LLMCandidateExpanderTests(unittest.TestCase):
    ...
    def test_read_repo_prompt_forces_lightweight_proposal_to_read_only(self) -> None:
        result = expand_candidates_with_llm(
            client=_client(
                {
                    "decisions": [
                        {
                            "title": "Inspect current repo implementation",
                            "recommendation": "Read the repo and rank the architecture paths.",
                            "why_now": ["The user asked for actual code evidence."],
                            "expected_result": "A source-backed recommendation.",
                            "downside": "Consumes read-only perception budget.",
                            "success_signal": "The answer cites implementation evidence.",
                            "confidence": 0.8,
                            "risk_level": "medium",
                            "execution_requested": True,
                            "handoff_task": "Read the local repo and decide the next path.",
                        }
                    ]
                }
            ),
            state=_state(),
            intent_text=(
                "请读取本地 /Users/demo/Spice-main 这个 repo 的当前实现，"
                "然后基于实际代码判断下一步。"
            ),
            rule_candidates=[_rule_candidate()],
        )
        ...
        self.assertFalse(candidate.execution_boundary.requires_confirmation)
        self.assertEqual(candidate.execution_intent.intent_class, "advisory")
        self.assertFalse(candidate.execution_intent.requested)
        self.assertEqual(candidate.execution_intent.required_permission_hint, "read_only")
        self.assertEqual(candidate.metadata["decision_mode"], "read_only_perception")
        self.assertTrue(candidate.metadata["read_only_intent_boundary_applied"])

    def test_read_repo_prompt_downgrades_legacy_execution_candidate(self) -> None:
        result = expand_candidates_with_llm(
            client=_client(
                {
                    "candidates": [
                        {
                            "action_type": "intent.execute",
                            "intent": "Read the repo and analyze the current implementation.",
                            "user_facing_title": "Inspect the repo",
                            "recommended_action": "Read implementation evidence before deciding.",
                            "target_refs": ["intent.main"],
                            "side_effect_class": "external_effect",
                            "requires_confirmation": True,
                            "availability_status": "needs_confirmation",
                            "why_blocked": ["Requires executor approval."],
                            "required_capability": "code_edit",
                            "executor_task": "Legacy executor task should be cleared.",
                            "execution_objective": "Legacy execution objective should be cleared.",
                            "execution_target": "executor boundary should be ignored",
                            "metadata": {
                                "executor_task": "Metadata executor task should be cleared.",
                                "handoff_task": "Metadata handoff should be cleared.",
                                "execution_objective": "Metadata execution objective should be cleared.",
                                "availability_status": "needs_confirmation",
                            },
                            "execution_intent": {
                                "intent_class": "execution_requested",
                                "requested": True,
                                "handoff_task": "Read the repo and report findings.",
        ...
        self.assertEqual(candidate.why_blocked, [])
        self.assertEqual(candidate.required_capability, "")
        self.assertEqual(candidate.execution_boundary.mode, "none")
        self.assertEqual(candidate.execution_boundary.target, "")
        self.assertFalse(candidate.execution_boundary.requires_confirmation)
        self.assertEqual(candidate.execution_boundary.side_effect_class, "read_only")
        self.assertEqual(candidate.execution_intent.intent_class, "advisory")
        self.assertFalse(candidate.execution_intent.requested)
        self.assertEqual(candidate.execution_intent.handoff_task, "")
        self.assertEqual(candidate.execution_intent.required_permission_hint, "read_only")
        self.assertEqual(candidate.metadata["decision_mode"], "read_only_perception")
        self.assertTrue(candidate.metadata["read_only_intent_boundary_applied"])
        self.assertEqual(candidate.metadata["executor_task"], "")
        self.assertNotIn("handoff_task", candidate.metadata)
        self.assertNotIn("execution_objective", candidate.metadata)
        self.assertNotIn("availability_status", candidate.metadata)

    def test_read_repo_prompt_sets_read_only_perception_policy(self) -> None:
        ...
        self.assertEqual(payload["decision_mode"], "read_only_perception")
        self.assertFalse(payload["decision_proposal_policy"]["execution_requested_default"])
        self.assertIn("execution_requested must be false", payload["decision_proposal_policy"]["instruction"])
